...
static int task_group_add_import_task(lua_State *L)
{
    ...
    task = rl_asset_ensure_async(path, NULL);
    if (task == 0) {
        return 0;
    }
    lua_settop(L, 4);
...
static int task_group_tick(lua_State *L)
{
    ...
    for (j = 0; j < g->n; j++) {
        ...
        if (!rl_asset_poll_task(e->task)) {
            continue;
        }
        e->rc = rl_asset_finish_task(e->task);
        rl_asset_free_task(e->task);
        e->task = 0;
        e->done = 1;
        g->completed_count++;
        if (e->rc != 0) {
...
void rl_register_lua_task_group(lua_State *L)
{
    ...
    lua_setfield(L, -2, "asset_create_task_group");
}
