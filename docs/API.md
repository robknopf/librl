...
- `rl_init()` / `rl_deinit()` bracket the entire runtime. Subsystem APIs require the runtime to be initialized. `rl_deinit()` destroys all subsystem state (assets, handles, lighting, cameras) — callers do not need to explicitly destroy individual resources before calling it.
