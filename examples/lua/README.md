...
- runs `boot.lua`, which sets `package.cpath`, bootstraps the loader, fetches the entry with `loader_import_asset_async` (coroutine + `loader_tick` until done), then loads the script with `loader_read_local` on the path from `loader_get_task_path` and runs the chunk (no `package.path` guessing).
