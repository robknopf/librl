...
5. **Client** polls `rl_asset_poll_task()` each frame until asset is fetched
6. **Client** calls `rl_asset_finish_task()` then creates the resource handle
...
- Uses `rl_asset_ensure_async()` → poll → finish → create handle
