...
int test_rl_fs_run(void)
{
    ...
    TEST_ASSERT(rl_asset_set_host(custom_host) == 0);
    TEST_ASSERT(strcmp(rl_asset_get_host(), custom_host) == 0);

    TEST_ASSERT(rl_fs_init_async("cache_fileio_test") == 0);
    poll_count = 0;
    while (!rl_fs_is_ready() && poll_count < 8) {
        rl_asset_tick();
        poll_count++;
    }
    TEST_ASSERT(rl_fs_is_ready() == true);
    TEST_ASSERT(test_raylib_callback_is_set());

    task = rl_fs_restore_async();
    TEST_ASSERT(task != 0);
    TEST_ASSERT(rl_asset_poll_task(task) == true);
    TEST_ASSERT(rl_asset_finish_task(task) == 0);
    ...
    task = rl_asset_ensure_async("assets/model.glb", "https://example.com/assets/model.glb?x=1#frag");
    TEST_ASSERT(task != 0);
    TEST_ASSERT(rl_asset_poll_task(task) == true);
    TEST_ASSERT(rl_asset_finish_task(task) == 0);
    rl_asset_free_task(task);
    ...
    TEST_ASSERT(rl_fs_exists("assets/model.glb") == true);
    ...
    task = rl_asset_ensure_async("assets/readme.txt", NULL);
    TEST_ASSERT(task != 0);
    TEST_ASSERT(rl_asset_poll_task(task) == true);
    TEST_ASSERT(rl_asset_finish_task(task) == 0);
    rl_asset_free_task(task);
    ...
    task = rl_asset_ensure_async("assets/scene.gltf", NULL);
    TEST_ASSERT(task != 0);
    TEST_ASSERT(rl_asset_poll_task(task) == false);
    TEST_ASSERT(rl_asset_poll_task(task) == false);
    TEST_ASSERT(rl_asset_poll_task(task) == true);
    TEST_ASSERT(rl_asset_finish_task(task) == 0);
    rl_asset_free_task(task);
    ...
    TEST_ASSERT(rl_fs_exists("assets/scene.gltf") == true);
    TEST_ASSERT(rl_fs_exists("assets/mesh.bin") == true);
    TEST_ASSERT(rl_fs_exists("assets/tex.png") == true);
...

    {
        ...
        task = rl_asset_ensure_many_async(batch_paths, 2);
        TEST_ASSERT(task != 0);
        poll_count = 0;
        while (!rl_asset_poll_task(task) && poll_count < 8) {
            poll_count++;
        }
    ...
    TEST_ASSERT(rl_fs_exists("assets/readme2.txt") == true);
    TEST_ASSERT(rl_fs_exists("assets/scene2.gltf") == true);
    TEST_ASSERT(rl_fs_exists("assets/mesh2.bin") == true);
    TEST_ASSERT(rl_fs_exists("assets/tex2.png") == true);
    ...
    rl_fs_deinit();
    TEST_ASSERT(rl_asset_set_host(NULL) == 0);
    TEST_ASSERT(!test_raylib_callback_is_set());

    // Re-init should succeed; the missing local .txt file stays absent.
    TEST_ASSERT(rl_fs_init("cache_fileio_test") == 0);
    TEST_ASSERT(test_raylib_callback_is_set());
