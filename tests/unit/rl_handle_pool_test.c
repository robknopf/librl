...
int test_rl_handle_pool_run(void)
{
    ...
    rl_handle_pool_init(&pool, RL_HANDLE_KIND_TEXTURE, 4, free_indices, 4, generations, occupied);
    ...
    TEST_ASSERT(RL_HANDLE_KIND(h2) == RL_HANDLE_KIND_TEXTURE);
    TEST_ASSERT(rl_handle_pool_resolve(&pool, h2, &resolved_index));
    TEST_ASSERT(resolved_index == RL_HANDLE_INDEX(h2));
    TEST_ASSERT(!rl_handle_pool_resolve(&pool, RL_HANDLE_MAKE(RL_HANDLE_KIND_COLOR, RL_HANDLE_INDEX(h2),
    ...
    TEST_ASSERT(RL_HANDLE_KIND(h1) == RL_HANDLE_KIND_TEXTURE);
